...
def view_modern_presentation(unique_id):
    ...
    from models import Collection, CollectionProperty, PresentationView, ExcelProperty
...
def manager_get_complex_favorites_list():
    ...
    # For testing without authentication, use manager_id = 1 or get from session if available
    manager_id = session.get('manager_id') or 1
    ...
    try:
        ...
        favorites_list = []
        for fav in favorites:
            # Ищем полные данные ЖК 
            complex_data = complexes_data.get(str(fav.complex_id))
            
            # Безопасный способ создания slug с fallback
            try:
                url = f"/zk/{create_slug(complex_data.name)}" if complex_data and complex_data.name else fav.complex_url or '#'
            except:
                url = fav.complex_url or '#'
            
            # Load real delivery_date and status from ResidentialComplex database
            from datetime import datetime
            ...
            if complex_data:
                ...
                # Primary: Use real database fields (ResidentialComplex model)
                if complex_data.end_build_year and complex_data.end_build_quarter:
                    delivery_date = f"{complex_data.end_build_quarter} кв. {complex_data.end_build_year} г."
                elif complex_data.end_build_year:
                    delivery_date = f"{complex_data.end_build_year} г."
                
                # Status logic using real database fields
                if complex_data.end_build_year:
                    # Determine status based on year
                    try:
                        if int(complex_data.end_build_year) <= current_year:
                            status = 'Сдан'
                        else:
                            status = 'Строится'
                    except (ValueError, TypeError):
            ...
            # Если нашли полные данные в базе, используем их
            if complex_data:
                # Подсчет зданий и квартир - через excel_data группировки
                buildings_count = 0
                # Подсчитаем корпуса по уникальным этажностям (как в строке 2921)
                complex_name_for_match = complex_data.name if complex_data else ''
                for name, data in excel_data.items():
                    if complex_name_for_match and complex_name_for_match.lower() in name.lower():
                        # Группируем по building_name (этажности)
                        buildings_count = data.get('buildings_count', 1)
                        break
                if buildings_count == 0:
                    buildings_count = len(complex_data.buildings) if complex_data and complex_data.buildings else 1
                excel_apartments = next((data['apartments_count'] for name, data in excel_data.items() 
                                       if complex_data and complex_data.name and complex_data.name.lower() in name.lower()), 0)
                db_apartments = sum(len(building.properties) for building in complex_data.buildings) if complex_data.buildings else 0
                apartments_count = excel_apartments or db_apartments
                
                favorites_list.append({
                    'id': str(complex_data.id),
                    'name': complex_data.name or fav.complex_name or 'ЖК без названия',
                    'developer': complex_data.developer.name if complex_data.developer else fav.developer_name or 'Застройщик не указан',
                    # Use real data from excel_properties aggregation (consistent with price matching)
                    'address': next((data['sample_address'] for name, data in excel_data.items() 
                                   if complex_data and complex_data.name and complex_data.name.lower() in name.lower()), None) 
                              or complex_data.sales_address or fav.complex_address or 'Адрес не указан',
                    'district': complex_data.district.name if complex_data.district else fav.district or 'Район не указан',
                    # Use real data from excel_properties aggregation (bypass broken favorites data)
                    'min_price': next((data['min_price'] for name, data in excel_data.items() 
                                     if complex_data and complex_data.name and complex_data.name.lower() in name.lower()), 0),
                    'max_price': next((data['max_price'] for name, data in excel_data.items() 
                                     if complex_data and complex_data.name and complex_data.name.lower() in name.lower()), 0),
                    'apartments_count': apartments_count,
                    # Load real images from excel_data photos field or use ResidentialComplex image
                    'image': next((data['photos'] for name, data in excel_data.items() 
                                 if complex_data and complex_data.name and complex_data.name.lower() in name.lower() and data.get('photos')), None)
                            or (complex_data.main_image if hasattr(complex_data, 'main_image') and complex_data.main_image 
                               else fav.complex_image or '/static/images/no-photo.jpg'),
                    'url': url,
                    'status': status,
                    'notes': fav.notes or '',
                    ...
                    'apartments_count': apartments_count,
                    'cashback_rate': complex_data.cashback_rate or 5.0
                })
            else:
                # Если полных данных нет, используем данные из избранного
                favorites_list.append({
                    'id': fav.complex_id or 'unknown',
                    'name': fav.complex_name or 'ЖК без названия',
                    'developer': fav.developer_name or 'Застройщик не указан',
                    'address': fav.complex_address or 'Адрес не указан',
                    'district': fav.district or 'Район не указан',
                    'min_price': fav.min_price or 0,
                    'max_price': fav.max_price or 0,
                    'image': fav.complex_image or '/static/images/no-photo.jpg',
                    'url': fav.complex_url or '#',
